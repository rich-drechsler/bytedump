...
#
#     sed -e '/^#$/d' -e '/^#[^!@].*/d' -e '/^  *#/d' bytedump-bash.sh
#
...
# also be important references. Fortunately, bash programmers don't need any of it.
...
#
#     LC_ALL=C ./bytedump-bash --text=unicode --debug=textmap,unexpanded /dev/null
#
# or
#
#     LC_ALL=C ./bytedump-bash --text=caret --debug=textmap,unexpanded /dev/null
#
...
#
#     LC_ALL=C ./bytedump-bash --text=unicode --debug=textmap /dev/null
#
# or
#
#     LC_ALL=C ./bytedump-bash --text=caret --debug=textmap /dev/null
#
...
#
#     ./bytedump-bash --debug=foreground,background,attributes,bytemap,textmap /dev/null
#
    ...
    if [[ $selector_output_name =~ ^SCRIPT_ATTRIBUTES_(BYTE|TEXT)_(BACKGROUND|FOREGROUND)$ ]]; then
        ...
        while [[ $selector_tokens =~ ^[$' \t']*([^$' \t'].*) ]]; do
            ...
            if [[ $selector_tokens =~ ^(0[xX]?)?[0-9a-fA-F] ]]; then
                ...
                # opening and closing string delimiters is accepted. After that the
                # individual characters represented by those bytes (in the user's
...
Initialize8_Attributes() {
    ...
    #
    #     ./bytedump-bash --debug=attributes,bytemap,textmap --text-foreground='red: [:digit:]' /dev/null
    #
    # or
    #
    #     ./bytedump-bash --debug=attributes,bytemap,textmap --foreground='red: [:digit:]' /dev/null
    #
...
LastEncodedByte() {
    #
    # An innocent "hack" that tries to recognize when 7-bit ASCII will be used to
    # display the dump that we generate. Really only tries to sync this script's
    # dump with dumps generated by other implementations (e.g., the Java version)
    # when any attributes are applied to ASCII encoded bytes. It's definitely not
    # a big deal, so this is good enough.
    #

    if [[ ${SCRIPT_LC_ALL[EXTERNAL]} == "C" || ${SCRIPT_LC_ALL[EXTERNAL]} == "POSIX" ]]; then
        printf "127\n"
    else
        printf "255\n"
    fi
...
#@# so if you run
#@#
#@#     bytedump-bash --addr=xxd file
#@#
#@# or equivalently
#@#
#@#     bytedump-bash --addr=hex:08 file
#@#
...
#@#
#@#     bytedump-bash --addr=decimal --byte=HEX --text=caret file
#@#
...
#@#
#@#     bytedump-bash --narrow --addr=decimal --byte=HEX --text=caret file
#@#
...
#@#
#@#     bytedump-bash --addr=decimal --byte=binary --text=empty file
#@#
#@# prints decimal addresses, binary (base 2) bytes, and hides the text field, while
#@#
#@#     bytedump-bash --addr=empty --byte=binary --text=empty file
#@#
